...
GPlotY::GPlotY(QObject *parent, QString uniqueIdentifierName /* = "" */)
...
/////////////////////////////////////////////////////////////////////
/*!
Populates a widget provided by a workbench in order to represent (and be adequately connected to) this GPlotY. 
...
void GPlotY::PopulateDeviceWidget(GDeviceWidget* theDeviceWidget )
{
    ...
    m_Dialog->setWindowTitle(tr("GPlotY Settings"));
	...
	DisplayTabLayout->addWidget(yGridOnLabel,2,0);
	DisplayTabLayout->addWidget(yGridOnCheckBox,2,1);
	DisplayTabLayout->addWidget(yGridColorLabel,3,0);
	DisplayTabLayout->addWidget(yGridColorButton,3,1);
	DisplayTabLayout->addWidget(xGridOnLabel,4,0);
	DisplayTabLayout->addWidget(xGridOnCheckBox,4,1);
	DisplayTabLayout->addWidget(xGridColorLabel,5,0);
	DisplayTabLayout->addWidget(xGridColorButton,5,1);
	...
	QLabel* filterYLabel = new QLabel(tr("Record every Nth update since Clear?"));
...
void GPlotY::EventNewValueY(double newValueY)
{	
	//Increment the number of updates: 
	m_NumUpdates = m_NumUpdates + 1;
	
	if(!m_FilterY||(!m_ExternalN&&m_NumUpdates%m_FilterYN==0)||(m_ExternalN&&m_NumUpdates%int(m_ExternalNBucket.DoubleValue()+0.5)==0))
